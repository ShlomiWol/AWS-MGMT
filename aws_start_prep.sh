...
# Showing all the instances of PREPROD
...
read -p "Are you sure you want to Start the instances($instances_count)? [y/n]: " answer
...
ids=`aws ec2 describe-instances --filter Name=tag:Environment,Values=PREPROD --query 'Reservations[*].Instances[*].[InstanceId, PrivateIpAddress, InstanceType, State.Name, Placement.AvailabilityZone, Tags[1].Value, Tags[0].Value]' --output text | awk '{print $1}'`
    ...
    while [ `aws ec2 describe-instances --instance-ids $ids --query 'Reservations[*].Instances[*].[InstanceId, State.Name]' --output text | grep running | wc -l` != $instances_count ]
    do
